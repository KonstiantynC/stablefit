...
(() => {
  ...
  const DEFAULT_LANG = "ua";
  const SUPPORTED_LANGS = ["ua", "en"];
  const LANG_KEY = "stablefit-language";

  const scriptEl = document.currentScript || document.querySelector('script[src*="main.js"]');
  const BASE_URL = new URL("../", new URL(scriptEl?.src ?? location.href, location.href));
  ...
  const i18n = (() => {
    ...
    async function load(lang) {
      if (!cache[lang]) {
        const res = await fetch(new URL(`locales/${lang}.json`, BASE_URL));
    ...
    function apply(dict) {
      document.querySelectorAll("[data-i18n]").forEach(el => {
        const text = get(dict, el.dataset.i18n);
        if (typeof text !== "string") return;
      ...
      [["[data-i18n-aria]",    "data-i18n-aria",    "aria-label"],
       ...
       ["[data-i18n-content]", "data-i18n-content", "content"]
        ...
        document.querySelectorAll(sel).forEach(el => {
          const text = get(dict, el.getAttribute(attr));
          if (text) el.setAttribute(prop, text);
        })
      );
    }

    function paintLang(lang) {
      document.querySelectorAll(".localisation-item[data-lang]")
    ...
    function paintNav() {
      ...
      const isCoach   = !path || path === "/" || /\/(coach(\.html)?)?\/?$/i.test(path) || last === "coach";
      const isClient  = /\/(for-clients|client)(\.html)?\/?$/i.test(path) || last === "for-clients" || last === "client";
      const isSupport = /\/support(\.html)?\/?$/i.test(path) || last === "support";

      document.querySelectorAll(".landing-link[data-landing]").forEach(link => {
        if (link.closest("footer")) { link.classList.remove("is-active"); return; }
        const l = link.dataset.landing;
        link.classList.toggle("is-active",
          (l === "coach" && isCoach) || (l === "client" && isClient) || (l === "support" && isSupport)
        );
      });
    }
    ...
    async function set(lang) {
      ...
      try {
        ...
        if (active !== dict) { active = dict; apply(dict); }
        paintLang(l);
        paintNav();
    ...
    function detect() {
      ...
      const browser = navigator.language?.slice(0, 2).toLowerCase() ?? DEFAULT_LANG;
      return SUPPORTED_LANGS.includes(browser) ? browser : DEFAULT_LANG;
    }

    return {
      set, detect, paintNav,
      apply: () => active && apply(active),
  ...
  const accordion = (() => {
    function setIndex(root, idx) {
      root.querySelectorAll(".f-slider-accordion-item").forEach((item, i) => {
        const open = i === idx;
        item.classList.toggle("is-open", open);
        item.querySelector(".f-slider-accordion-trigger")?.setAttribute("aria-expanded", String(open));
      });
    }

    function init() {
      document.querySelectorAll("[data-f-slider-accordion]").forEach(root => {
        if (root.dataset.sfAccordionBound) return;
        root.dataset.sfAccordionBound = "1";
        root.querySelectorAll(".f-slider-accordion-item").forEach((item, i) => {
          item.querySelector(".f-slider-accordion-trigger")?.addEventListener("click", () => {
            if (item.classList.contains("is-open")) {
              item.classList.remove("is-open");
              item.querySelector(".f-slider-accordion-trigger").setAttribute("aria-expanded", "false");
            } else {
              setIndex(root, i);
            }
          });
  ...
  function initScrollSync() {
    ...
    const root  = row.querySelector("[data-f-slider-accordion]");
    const left  = row.querySelector(".content-left");
    const right = row.querySelector(".content-right");
    const wraps = [...row.querySelectorAll(".content-right-image-wrp")];
    const items = root ? [...root.querySelectorAll(".f-slider-accordion-item")] : [];

    if (!root || !left || !right || !wraps.length) return null;

    const mq = matchMedia("(max-width: 1023px)");
    let lastIdx = -1, ignore = false;

    function leftCenterY() {
      const r = left.getBoundingClientRect();
      const top = Math.max(0, r.top), bot = Math.min(innerHeight, r.bottom);
      return bot > top ? (top + bot) / 2 : innerHeight * 0.45;
    }

    function bestDesktopIdx() {
      const mid = innerHeight * 0.3, half = Math.min(120, innerHeight * 0.11);
      let best = 0, bestOv = -1;
      wraps.forEach((w, i) => {
        const r = w.getBoundingClientRect();
        const ov = Math.max(0, Math.min(r.bottom, mid + half) - Math.max(r.top, mid - half));
      ...
      while (best > 0 && wraps[best].getBoundingClientRect().top > innerHeight * 0.3) best--;
      return best;
    }

    function bestCarouselIdx() {
      const cr = right.getBoundingClientRect(), mid = cr.left + cr.width / 2;
    ...
    function scrollToWrap(idx) {
      const wrap = wraps[idx];
      if (!wrap) return;
      const r = wrap.getBoundingClientRect();
      const delta = (r.top + r.height / 2) - leftCenterY();
      if (Math.abs(delta) < 3) return;
      lastIdx = idx; ignore = true;
      scrollBy({ top: delta, behavior: "smooth" });
      const unlock = () => { ignore = false; };
    ...
    function scrollCarousel(idx) {
      if (!wraps[idx]) return;
      lastIdx = idx; ignore = true;
      right.scrollTo({ left: wraps[idx].offsetLeft, behavior: "smooth" });
      setTimeout(() => { ignore = false; }, 500);
    }

    function syncDesktop() {
      if (ignore || mq.matches) return;
      const rr = row.getBoundingClientRect();
      if (rr.bottom < 0 || rr.top > innerHeight) return;
      let idx = bestDesktopIdx();
      const r0 = wraps[0].getBoundingClientRect();
      if (r0.height > 0 && Math.max(0, Math.min(r0.bottom, innerHeight) - Math.max(r0.top, 0)) / r0.height > 0.38) idx = 0;
      if (idx === lastIdx) return;
      lastIdx = idx;
      accordion.setIndex(root, idx);
    }

    function syncCarousel() {
      if (ignore || !mq.matches) return;
      const idx = bestCarouselIdx();
      if (idx === lastIdx) return;
      lastIdx = idx;
      accordion.setIndex(root, idx);
    }

    function runSync() { lastIdx = -1; mq.matches ? syncCarousel() : syncDesktop(); }

    // Accordion click → scroll to image
    const clickHandlers = items.map((item, i) => {
      const trigger = item.querySelector(".f-slider-accordion-trigger");
      if (!trigger) return null;
      const fn = () => queueMicrotask(() => {
        if (!item.classList.contains("is-open") || i >= wraps.length) return;
        mq.matches ? scrollCarousel(i) : scrollToWrap(i);
      });
    ...
    const ro = typeof ResizeObserver !== "undefined"
      ? new ResizeObserver(() => { mq.matches ? syncCarousel() : syncDesktop(); })
      : null;
    ro?.observe(row);

    const io = typeof IntersectionObserver !== "undefined"
      ? new IntersectionObserver(
          () => { if (!ignore && !mq.matches) syncDesktop(); },
          { threshold: Array.from({ length: 21 }, (_, i) => i / 20), rootMargin: "-38% 0px -38% 0px" }
        )
      : null;
    wraps.forEach(w => io?.observe(w));

    const onScroll    = () => { if (!mq.matches) syncDesktop(); };
    const onScrollEnd = () => { mq.matches ? syncCarousel() : syncDesktop(); };

    addEventListener("scroll", onScroll, { passive: true });
    addEventListener("scrollend", onScrollEnd);
    addEventListener("resize", runSync);
    right.addEventListener("scroll", syncCarousel, { passive: true });
    mq.addEventListener("change", runSync);
    requestAnimationFrame(runSync);
    addEventListener("load", runSync, { once: true });
    ...
    return () => {
      ...
      removeEventListener("resize", runSync);
      right.removeEventListener("scroll", syncCarousel);
      mq.removeEventListener("change", runSync);
      clickHandlers.forEach(([t, fn]) => t.removeEventListener("click", fn));
    };
  }
  ...
  function initMarquee() {
    document.querySelectorAll(".testimonials-marquee-track").forEach(track => {
      if (track.dataset.infinitePrepared) return;
      const originals = [...track.children];
      if (!originals.length) return;

      const group = Object.assign(document.createElement("div"), { className: "testimonials-marquee-group" });
      originals.forEach(c => group.appendChild(c));
      track.appendChild(group);

      const mw = track.closest(".testimonials-marquee")?.clientWidth ?? 0;
      if (mw > 0) {
        const base = [...group.children];
        let n = 0;
        while (group.scrollWidth < mw && n++ < 50)
          base.forEach(c => { const cl = c.cloneNode(true); cl.setAttribute("aria-hidden", "true"); group.appendChild(cl); });
      }
  ...
  function initStagger() {
    ...
    const sections = [...document.querySelectorAll("main > section")].slice(0, 2);
    if (!sections.length) return null;

    const SEL1 = ".section-name,.section-title,.gradient-title,.gradient_cl-title,.section-description,.content-text,.card,.features-cart,.f-slider-mobile-item,.progress-cart,.how-to-start-card,.automatisation-card,.testimonial-card,.pricing-card,.download-app-card,.try-button,.download-button,.image-wrp,.content-right-image-wrp img,.progress-cart-image img,.automatisation-card-image img,.how-to-start-card img,.download-app-card img";
    const SEL2 = ".section-name,.section-title,.gradient-title,.gradient_cl-title,.section-description";
    ...
    sections.forEach((section, si) => {
      section.classList.add("stagger-parent");
      const seen = new Set();
      const items = [...section.querySelectorAll(si === 0 ? SEL1 : SEL2)].filter(el => {
        if (seen.has(el)) return false;
        if (si === 0 && el.closest("[data-f-slider-scroll-sync]") && !el.closest(".f-slider-mobile-layout")) return false;
        return seen.add(el);
      });
      items.forEach((el, i) => { el.classList.add("stagger-item"); el.style.setProperty("--i", i); });

      if (si === 0 && section.classList.contains("hero")) {
        const title = section.querySelector(".section-title");
        const img   = section.querySelector(".image-wrp");
        if (title && img) img.style.setProperty("--i", items.indexOf(title));
      }
      observer.observe(section);
    });
  ...
  function initMobileNav() {
    ...
    const open  = () => { document.body.classList.add("nav-open");    burger.setAttribute("aria-expanded", "true");  drawer.setAttribute("aria-hidden", "false"); };
    const close = () => { document.body.classList.remove("nav-open"); burger.setAttribute("aria-expanded", "false"); drawer.setAttribute("aria-hidden", "true");  };

    burger.addEventListener("click", () => document.body.classList.contains("nav-open") ? close() : open());
    document.querySelector(".header-drawer-close")?.addEventListener("click", close);
    document.addEventListener("keydown", e => { if (e.key === "Escape" && document.body.classList.contains("nav-open")) close(); });
    drawer.querySelectorAll("a[href], .localisation-item[data-lang]").forEach(el => el.addEventListener("click", close));
  }
  ...
  function initScrollToDownload() {
    ...
    const handlers = [];
    document.querySelectorAll("a.download-button, a.try-button").forEach(link => {
      const fn = e => { e.preventDefault(); target.scrollIntoView({ behavior: "smooth", block: "start" }); };
      link.addEventListener("click", fn);
      handlers.push([link, fn]);
    });
    return () => handlers.forEach(([l, fn]) => l.removeEventListener("click", fn));
  }

  // ─── SPA navigation ───────────────────────────────────────────────────────────
  const spa = (() => {
    function getLanding(pathname) {
      const path = pathname.replace(/\/index\.html$/i, "");
      if (/\/for-clients\/?$/i.test(path)) return "client";
      if (!path || path === "/" || /\/coach\/?$/i.test(path)) return "coach";
      return null;
    ...
    async function swap(url, push = true) {
      ...
      const next = doc.querySelector("main"), curr = document.querySelector("main");
      if (!next || !curr) throw new Error("Main not found");
      curr.replaceWith(next);

      const isClient = doc.body.classList.contains("page-client");
      document.body.classList.toggle("page-client", isClient);
      document.body.classList.toggle("page-coach", doc.body.classList.contains("page-coach"));
      document.body.classList.remove("nav-open");
      document.querySelectorAll("header .download-button").forEach(btn => {
        btn.classList.toggle("btn-cl", isClient);
        btn.classList.toggle("btn", !isClient);
      });

      if (doc.title) document.title = doc.title;
      if (push) history.pushState({ sfLandingSwap: true }, "", url);
      i18n.apply();
      i18n.paintNav();
      modules.init();
    ...
    function init() {
      ...
      document.addEventListener("click", async e => {
        const link = e.target.closest("a.landing-link[data-landing]");
        if (!link || link.closest("footer")) return;
        const landing = link.dataset.landing;
        if (landing !== "coach" && landing !== "client") return;
        ...
        if (getLanding(location.pathname) === getLanding(next.pathname)) return;

        busy = true;
        try { await swap(next.href); }
        catch { location.href = next.href; }
        finally { busy = false; }
      });

      addEventListener("popstate", async () => {
        if (getLanding(location.pathname)) try { await swap(location.href, false); } catch {}
      });
    }
  ...
  const modules = (() => {
    ...
    return {
      init() {
        ...
        accordion.init();
        initMarquee();
        const cleanups = [initScrollSync(), initScrollToDownload(), initStagger()].filter(Boolean);
        teardown = () => cleanups.forEach(fn => { try { fn(); } catch {} });
      }
    };
  })();
  ...
  document.querySelectorAll(".localisation-item[data-lang]").forEach(btn => {
    const change = () => i18n.set(btn.dataset.lang).then(initMarquee);
    btn.addEventListener("click", change);
    btn.addEventListener("keydown", e => { if (e.key === "Enter" || e.key === " ") { e.preventDefault(); change(); } });
  });

  i18n.set(i18n.detect()).then(initMarquee).catch(initMarquee);
})();